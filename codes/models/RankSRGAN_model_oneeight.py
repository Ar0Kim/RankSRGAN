...
import argparse
#from codes.train.main.args import version as version
...
class SRGANModel(BaseModel):
    def __init__(self, opt):
        ...
        first_iter = opt['train']['niter']//4
        second_iter = opt['train']['niter']//4*2
        third_iter = opt['train']['niter']//4*3

        self.first_iter = first_iter
        self.second_iter = second_iter
        self.third_iter = third_iter

        #version = config.version
        #config = get_config()
        version = opt['network_D']['version']
        #version = opt['version']
    ...
    def optimize_parameters(self, step):
        ...
        epoch_flag = 0
        if (step == self.first_iter or step == self.second_iter or step == self.third_iter) and (epoch_flag ==0):
            print(f"PGD{self.version}")
            epoch_flag = 1
            self.version = self.version + 1
            self.netD = networks.define_D(self.opt).to(self.device)
            if self.opt['dist']:
                self.netD = DistributedDataParallel(self.netD,
                                                    device_ids=[torch.cuda.current_device()])
            else:
